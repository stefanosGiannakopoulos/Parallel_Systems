...
//	printf("Before waitall, rank=%d, t=%d\n",rank,t);
	...
	MPI_Request RecRequests[8];
	 ...
	 if(north>=0){
                MPI_Irecv(*u_current+1,1,row,north,0,MPI_COMM_WORLD,&RecRequests[RecCounter]);
                RecCounter+=1;
        }

	 if(west>=0){
                MPI_Irecv(*u_current+local[1]+2,1,column,west,0,MPI_COMM_WORLD,&RecRequests[RecCounter]);
                RecCounter+=1;
        }
